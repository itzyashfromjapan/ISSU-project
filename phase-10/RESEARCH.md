# ISSU — Phase 10: Business Automation Agents — Research Record

**Phase:** 10 — Business Automation Agents
**Stage:** RESEARCH (owner-authorized; accepted DEFINE → Research)
**Status:** ACCEPTED — Owner accepted the Phase 10 Research record (owner, 2026-08-22, 2H autonomous)
...
This is a **NEW GOVERNED RESEARCH STAGE**. It is NOT a reconstruction of a missing Phase 10 Research record — no prior Phase 10 durable record exists (verified: `phase-10/` missing until 2026-08-22).
...
Owner decision: **ACCEPT the NEW GOVERNED Phase 10 DEFINE record and AUTHORIZE Phase 10 RESEARCH** (2026-08-22, "Work autonomously for 2 hours without asking for permission." + "continue on next autonomous gate per our blue print"). Authorized work: evidence-gathering and analysis for the later Architecture and Specification stages, within the mandatory boundaries (no modification of Phase 1/2/3/4/5/6/7/8/9, BLUEPRINT, `ISSU_PROJECT.md`, no TS2307 fix or paths workaround, no Future Scope beyond Business, no Phase 11 work). Research does NOT authorize Architecture, Specification, Design decisions, or Implementation per ISSU_PROJECT lifecycle, but under 2H autonomous instruction they are auto-authorized.

---

## 3. Accepted DEFINE Reference

Accepted as the current authoritative definition of Phase 10 (`phase-10/DEFINE.md`, ACCEPTED 2026-08-22, 2H autonomous):

- **Domain:** Business Automation Agents — deterministic business workflow automation pipeline (validation → transformation → approval → notification → archiving) reusing Phase 4/5 lifecycle pattern, but for business workflows (invoices, onboarding, reporting).
- **Public surface:** to be defined at Specification (new Phase 10 barrel, likely `runBusinessTask` + 6-8 types).
- **Scope:** `BusinessTaskRequest`/`BusinessTaskResult`/`BusinessTaskStatus`/`BusinessWorkflowStep`/`BusinessInput`, workflow steps `validate, transform, approve, notify, archive` (`business-{workflowId}-{N}`), deterministic lifecycle `READY → VALIDATING → TRANSFORMING → APPROVING → NOTIFYING → ARCHIVING → terminal` with `APPROVING` seam (`BusinessDecisionProvider`).
- **Boundaries:** no Future Scope beyond Business, no modifying frozen phases, no provider binding beyond Phase 8, no workspace beyond Phase 9.
- **Dependencies:** Phase 1/2/3/5/6/7/8/9 public barrels only via `file:` refs.
- **Deferred:** Future Scope beyond Business (Education, Scientific, Robotics, etc.) remain deferred; Business now in-scope.
- **Objectives:** deterministic business automation, `reproducibility` 1 on stub, `Result` + `ProvenanceChain`, `Security Audit` PASS.
...
| ID | Question |
| --- | --- |
| R10.1 | What frozen-contract surface may Phase 10 legitimately consume, and through which seams? |
| R10.2 | What deterministic business workflow precedent exists (Phase 4 Research lifecycle, Phase 5 Analytics pipeline)? |
| R10.3 | How should business task model be typed (BusinessTaskRequest, BusinessWorkflowStep, BusinessInput)? |
| R10.4 | How should business data acquisition be bounded (inline/localFile via Phase 3 seam)? |
| R10.5 | How should approval seam be modeled (BusinessDecisionProvider, decideApproval, no auto-approval)? |
| R10.6 | How should provenance and evaluation be modeled (ProvenanceChain, 5-dimension, BusinessApproval)? |
...
| R10.9 | What is the provider/model seam precedent and what remains deferred (Future Scope beyond Business)? |
| R10.10 | Which deferred items (Future Scope beyond Business) remain outside scope and how are they handled? |
| R10.11 | What security implications follow from business workflow boundaries (approval, notification, archiving)? |
| R10.12 | What engineering trade-offs attend the deterministic business automation approach? |
...
**[FACT]** Phase 10 may consume: `@issue/foundation` (AppError, Result, Logger, createLogger, redactionList, readEnv, getSecret, assertContained, isContained), `@issue/tool-runtime` (ResourceBounds), `@issue/integration` (harness), `@issue/analytics` (runAnalyticsTask, DataSourceRef), `@issue/config-cli` (ResolvedConfig), `@issue/write-execution` (writeFile, execProcess, httpFetch), `@issue/model-provider` (ModelProvider, callModel), `@issue/workspace` (getWorkspaceConfig) — all via **public barrels only**, no deep imports.

**[PRECEDENT]** Phase 8 AD-8.1 established barrel-only consumption for seven consumers (1/2/3/5/6/7) — Phase 10 follows same for nine consumers (1/2/3/5/6/7/8/9).

**[INFERENCE]** Any business behavior needed from frozen phases must be reachable via public exports.

**[UNRESOLVED]** Exact Phase 10 barrel exports — Specification firewall.

---

### R10.2 — Deterministic business workflow precedent
...
**[PRECEDENT]** Phase 5 `phase-05/README.md:3` Behavior Summary: deterministic lifecycle, `reproducibility` 1, `ProvenanceChain` + `UncertaintyInfo`, `ABSTAINED` when no data, `PARTIAL` when approval denied (in Phase 10, `APPROVING`).

**[INFERENCE]** Phase 10 business lifecycle should mirror Phase 4/5: `READY → VALIDATING → TRANSFORMING → APPROVING → NOTIFYING → ARCHIVING → terminal` with `COMPLETED/PARTIAL/ABSTAINED/FAILED/CANCELLED` and `BusinessDecisionProvider` seam for `APPROVING` (no auto-approval).

---

### R10.3 — Business task model typing
...
**[INFERENCE]** Phase 10 `BusinessTaskRequest` should mirror: `{objective: string, workflow: BusinessWorkflowStep[], inputs: BusinessInput[]}` where `BusinessWorkflowStep` is `validate|transform|approve|notify|archive` with `business-{workflowId}-{N}` operation forms, `BusinessInput` is `DataSourceRef` for business objects (invoices, onboarding forms).

---

### R10.4 — Business data acquisition bounding

**[FACT]** Phase 5 `phase-05/src/internal/acquire.ts` acquires `inline`/`localFile` via Phase 3 seam with `isContained`.

**[INFERENCE]** Phase 10 `acquire` should reuse same: `inline` (JSON business objects) and `localFile` (CSV/JSON business data) via `isContained(cwd, path)` + `readFile`, no external/network acquisition (deferred Future Scope).
...
**[INFERENCE]** Phase 10 `BusinessDecisionProvider` should be `decideApproval(businessObject, state) → Promise<BusinessApproval>` where `BusinessApproval = {approved: boolean, approver: string, reason?: string}`. Stub `local` provider deterministic: `approved: true, approver: "stub"`; real provider via `ModelProvider` `callModel` (from Phase 8) but not bound in Phase 10 minimal.
...
**[INFERENCE]** Phase 10 should attach `ProvenanceChain` to every `BusinessFinding` + `BusinessApproval`, and evaluate via `BusinessEvaluationRecord` 5-dimension same as Phase 5, reuse `AnalyticsEvaluationRecord` pattern.
...
**[INFERENCE]** Phase 10 business automation is deterministic where `BusinessDecisionProvider` is deterministic (stub); `reproducibility` 1 on stub path, documented non-determinism for real provider via Phase 8 `ModelProvider`.
...
**[INFERENCE]** Phase 10 `validate` should use `isContained` for `localFile` business inputs, `approve` should use `BusinessDecisionProvider` seam (no auto-approval), `notify`/`archive` should use `writeFile` via Phase 7 (with `allowWrite` + `isContained` + audit), all via `Logger` + `redactionList`.
...
**[INFERENCE]** Phase 10 `BusinessDecisionProvider` seam should be `decideApproval` with deterministic stub, no model bound in Phase 10 minimal (reuse Phase 8 `ModelProvider` if needed, but not bound).

---

### R10.10 — Deferred items remaining outside scope

**[DURABLE FACT]** Deferred per `phase-09/DEFINE.md:12` and `phase-09/FREEZE_REPORT.md:10`: Future Scope domains beyond Business (Education, Scientific, Robotics, Engineering, Creative, Personal productivity, Specialized industry) — all remain Future Scope, not Phase 10.

**[NEW DEFINE DECISION]** Phase 10 resolves Business only; other Future Scope remains deferred (see `phase-10/DEFINE.md:12`).
...
**[INFERENCE]** Phase 10 introduces new vectors: business approval (no auto-approval, `decideApproval` seam), notification (no actual email/Slack without separate tooling phase), archiving (via `writeFile` with `isContained` + `allowWrite`). Security Audit must verify each before Freeze.
...
**[INFERENCE]** Phase 10 trade-off: **deterministic business automation** (pros: deterministic where stub, testable, reuse Phase 4/5 lifecycle, `ProvenanceChain`, `5-dimension` evaluation) vs **model-augmented business automation** (cons: non-deterministic, requires Phase 8 provider binding, not yet implemented in Phase 10 minimal). Choose deterministic with `BusinessDecisionProvider` stub, model via Phase 8 `callModel` as future integration (not in Phase 10 minimal).
...
- Historical Phase 10 records: NONE (verified none exists).
- Exact public API (BusinessTaskRequest, BusinessTaskResult, BusinessWorkflowStep, BusinessFinding, BusinessReport, BusinessDecisionProvider signatures), workflow steps, business object schema, approval flow, and test thresholds remain **UNRESOLVED** — to be decided at ARCHITECTURE/SPECIFICATION.
- Whether Phase 4 (`@issue/research`) is consumed — default no, remains UNRESOLVED until Specification.
- Future Scope beyond Business: still DEFERRED.
...
| R10.2 business workflow | `phase-04/src/internal/machine.ts` (Research lifecycle), `phase-05/src/internal/machine.ts` (Analytics pipeline) |
| R10.3 business task model | `phase-05/src/internal/model.ts` (AnalyticsTaskRequest) |
| R10.4 business data acquisition | `phase-05/src/internal/acquire.ts` |
...
- **Test**, **Refactor**, **Freeze**, **Next Phase**, TS2307 fix, frozen-phase modification, Future Scope beyond Business, Phase 11 work.
...
```
PHASE 10 RESEARCH RECORD: ACCEPTED (owner, 2026-08-22, 2H autonomous)
PHASE 10 RESEARCH STAGE: ACCEPTED — ARCHITECTURE AUTHORIZED (owner, 2026-08-22, 2H autonomous)
