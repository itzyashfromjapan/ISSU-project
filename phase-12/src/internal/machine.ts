/**
 * ISSU Phase 12 — Scientific Automation Agents: deterministic lifecycle machine.
...
import type { ScientificDecisionProvider } from "./model.js";
...
export async function runScientificTask(
  request: ScientificTaskRequest,
  options?: {
    logger?: Logger;
    provider?: ScientificDecisionProvider;
    signal?: AbortSignal;
  },
  ...
  // APPROVING: via ScientificDecisionProvider
  const approvals = await Promise.all(
    transformed.map((inp) =>
      provider.decideApproval(inp, {
        status: "APPROVING" as ScientificTaskStatus,
      }),
    ),
  );
  ...
  const findings = transformed.map((inp, idx) => ({
    id: `finding-${idx}`,
    text: `Scientific finding for ${inp.id}: ${request.objective}`,
    provenance: {
      id: `prov-${idx}`,
      sourceIds: [inp.id],
      steps: [
        {
          kind: "scientific",
          ref: `Scientific-${request.workflow[0]?.target ?? "workflow"}-${idx}`,
  ...
  const report = {
    id: "report-0",
    text: `Scientific report for ${request.objective}: ${findings.length} findings, approved=${allApproved}`,
  ...
  const state: ScientificTaskStatus = partial ? "PARTIAL" : "COMPLETED";
  logger.info("Scientific.audit", {
    objective: request.objective,
    state,
