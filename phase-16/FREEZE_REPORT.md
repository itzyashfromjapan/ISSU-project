...
- **D1 conformance rework:** pre-rework, this phase's src/ was byte-identical to Phase 10 and exported runBusinessTask / BusinessTask*, which did NOT conform to this phase's accepted SPECIFICATION surface (runProductivityTask / ProductivityTask*). Under Owner authorization D1 (2026-08-22) all public symbols, internal identifiers, the audit event (Productivity.audit), the provenance ref prefix (Productivity-{workflowId}-{N}), tests, and docs were corrected to the phase-specific API. Behavior remains the same deterministic lifecycle; no capability was invented beyond the SPEC.
