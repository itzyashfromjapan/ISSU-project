/**
 * ISSU Phase 16 — Productivity Automation Agents: deterministic lifecycle machine.
...
import type { ProductivityDecisionProvider } from "./model.js";
...
export async function runProductivityTask(
  request: ProductivityTaskRequest,
  options?: {
    logger?: Logger;
    provider?: ProductivityDecisionProvider;
    signal?: AbortSignal;
  },
): Promise<ProductivityTaskResult> {
  const logger =
  ...
  if (validInputs.length === 0) {
    logger.info("Productivity.audit", {
  ...
  // APPROVING: via ProductivityDecisionProvider
  const approvals = await Promise.all(
    transformed.map((inp) =>
      provider.decideApproval(inp, {
        status: "APPROVING" as ProductivityTaskStatus,
      }),
    ),
  );
  ...
  const findings = transformed.map((inp, idx) => ({
    id: `finding-${idx}`,
    text: `Productivity finding for ${inp.id}: ${request.objective}`,
    provenance: {
      id: `prov-${idx}`,
      sourceIds: [inp.id],
      steps: [
        {
          kind: "productivity",
          ref: `Productivity-${request.workflow[0]?.target ?? "workflow"}-${idx}`,
  ...
  const report = {
    id: "report-0",
    text: `Productivity report for ${request.objective}: ${findings.length} findings, approved=${allApproved}`,
  ...
  const state: ProductivityTaskStatus = partial ? "PARTIAL" : "COMPLETED";
  logger.info("Productivity.audit", {
    objective: request.objective,
    state,
