/**
 * ISSU Phase 15 — Creative Automation Agents: deterministic lifecycle machine.
...
import type { CreativeDecisionProvider } from "./model.js";
...
export async function runCreativeTask(
  ...
  options?: {
    logger?: Logger;
    provider?: CreativeDecisionProvider;
    signal?: AbortSignal;
  },
  ...
  // APPROVING: via CreativeDecisionProvider
  const approvals = await Promise.all(
    transformed.map((inp) =>
      provider.decideApproval(inp, {
        status: "APPROVING" as CreativeTaskStatus,
      }),
    ),
  );
  ...
  const findings = transformed.map((inp, idx) => ({
    id: `finding-${idx}`,
    text: `Creative finding for ${inp.id}: ${request.objective}`,
    provenance: {
      id: `prov-${idx}`,
      sourceIds: [inp.id],
      steps: [
        {
          kind: "creative",
          ref: `Creative-${request.workflow[0]?.target ?? "workflow"}-${idx}`,
  ...
  const report = {
    id: "report-0",
    text: `Creative report for ${request.objective}: ${findings.length} findings, approved=${allApproved}`,
  ...
  const state: CreativeTaskStatus = partial ? "PARTIAL" : "COMPLETED";
  logger.info("Creative.audit", {
    objective: request.objective,
    state,
