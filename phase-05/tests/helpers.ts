...
import type { AnalyticsDecisionProvider } from "../src/index.js";

export async function makeRoot(prefix = "p5-"): Promise<string> {
...
export interface RecordingAnalyticsProvider {
  readonly provider: AnalyticsDecisionProvider;
  readonly calls: {
    readonly selectSource: number;
    readonly selectFindingToVerify: number;
  };
}

export function recordingAnalyticsProvider(
  base: AnalyticsDecisionProvider,
): RecordingAnalyticsProvider {
  const calls = { selectSource: 0, selectFindingToVerify: 0 };
  return {
    provider: {
      ...
      async selectFindingToVerify(findings, state) {
        calls.selectFindingToVerify += 1;
        return base.selectFindingToVerify(findings, state);
      },
      async decideRefinement(refinements, state) {
        return base.decideRefinement(refinements, state);
      },
    },
    calls,
  };
