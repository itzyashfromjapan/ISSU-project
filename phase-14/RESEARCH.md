# ISSU — Phase 14: Engineering Agents — Research Record

**Phase:** 10 — Engineering Agents
**Stage:** RESEARCH (owner-authorized; accepted DEFINE → Research)
**Status:** ACCEPTED — Owner accepted the Phase 14 Research record (owner, 2026-08-22, 2H autonomous)
...
This is a **NEW GOVERNED RESEARCH STAGE**. It is NOT a reconstruction of a missing Phase 14 Research record — no prior Phase 14 durable record exists (verified: `phase-10/` missing until 2026-08-22).
...
Owner decision: **ACCEPT the NEW GOVERNED Phase 14 DEFINE record and AUTHORIZE Phase 14 RESEARCH** (2026-08-22, "Work autonomously for 2 hours without asking for permission." + "continue on next autonomous gate per our blue print"). Authorized work: evidence-gathering and analysis for the later Architecture and Specification stages, within the mandatory boundaries (no modification of Phase 1/2/3/4/5/6/7/8/9, BLUEPRINT, `ISSU_PROJECT.md`, no TS2307 fix or paths workaround, no Future Scope beyond Engineering, no Phase 14 work). Research does NOT authorize Architecture, Specification, Design decisions, or Implementation per ISSU_PROJECT lifecycle, but under 2H autonomous instruction they are auto-authorized.

---

## 3. Accepted DEFINE Reference

Accepted as the current authoritative definition of Phase 14 (`phase-10/DEFINE.md`, ACCEPTED 2026-08-22, 2H autonomous):

- **Domain:** Engineering Agents — deterministic Engineering workflow automation pipeline (validation → transformation → approval → notification → archiving) reusing Phase 4/5 lifecycle pattern, but for Engineering workflows (invoices, onboarding, reporting).
- **Public surface:** to be defined at Specification (new Phase 14 barrel, likely `runEngineeringTask` + 6-8 types).
- **Scope:** `EngineeringTaskRequest`/`EngineeringTaskResult`/`EngineeringTaskStatus`/`EngineeringWorkflowStep`/`EngineeringInput`, workflow steps `validate, transform, approve, notify, archive` (`Engineering-{workflowId}-{N}`), deterministic lifecycle `READY → VALIDATING → TRANSFORMING → APPROVING → NOTIFYING → ARCHIVING → terminal` with `APPROVING` seam (`EngineeringDecisionProvider`).
- **Boundaries:** no Future Scope beyond Engineering, no modifying frozen phases, no provider binding beyond Phase 8, no workspace beyond Phase 9.
- **Dependencies:** Phase 1/2/3/5/6/7/8/9 public barrels only via `file:` refs.
- **Deferred:** Future Scope beyond Engineering (Engineering, Engineering, Robotics, etc.) remain deferred; Engineering now in-scope.
- **Objectives:** deterministic Engineering automation, `reproducibility` 1 on stub, `Result` + `ProvenanceChain`, `Security Audit` PASS.
...
| ID | Question |
| --- | --- |
| R10.1 | What frozen-contract surface may Phase 14 legitimately consume, and through which seams? |
| R10.2 | What deterministic Engineering workflow precedent exists (Phase 4 Research lifecycle, Phase 5 Analytics pipeline)? |
| R10.3 | How should Engineering task model be typed (EngineeringTaskRequest, EngineeringWorkflowStep, EngineeringInput)? |
| R10.4 | How should Engineering data acquisition be bounded (inline/localFile via Phase 3 seam)? |
| R10.5 | How should approval seam be modeled (EngineeringDecisionProvider, decideApproval, no auto-approval)? |
| R10.6 | How should provenance and evaluation be modeled (ProvenanceChain, 5-dimension, EngineeringApproval)? |
...
| R10.9 | What is the provider/model seam precedent and what remains deferred (Future Scope beyond Engineering)? |
| R10.10 | Which deferred items (Future Scope beyond Engineering) remain outside scope and how are they handled? |
| R10.11 | What security implications follow from Engineering workflow boundaries (approval, notification, archiving)? |
| R10.12 | What engineering trade-offs attend the deterministic Engineering automation approach? |
...
**[FACT]** Phase 14 may consume: `@issue/foundation` (AppError, Result, Logger, createLogger, redactionList, readEnv, getSecret, assertContained, isContained), `@issue/tool-runtime` (ResourceBounds), `@issue/integration` (harness), `@issue/analytics` (runAnalyticsTask, DataSourceRef), `@issue/config-cli` (ResolvedConfig), `@issue/write-execution` (writeFile, execProcess, httpFetch), `@issue/model-provider` (ModelProvider, callModel), `@issue/workspace` (getWorkspaceConfig) — all via **public barrels only**, no deep imports.

**[PRECEDENT]** Phase 8 AD-8.1 established barrel-only consumption for seven consumers (1/2/3/5/6/7) — Phase 14 follows same for nine consumers (1/2/3/5/6/7/8/9).

**[INFERENCE]** Any Engineering behavior needed from frozen phases must be reachable via public exports.

**[UNRESOLVED]** Exact Phase 14 barrel exports — Specification firewall.

---

### R10.2 — Deterministic Engineering workflow precedent
...
**[PRECEDENT]** Phase 5 `phase-05/README.md:3` Behavior Summary: deterministic lifecycle, `reproducibility` 1, `ProvenanceChain` + `UncertaintyInfo`, `ABSTAINED` when no data, `PARTIAL` when approval denied (in Phase 14, `APPROVING`).

**[INFERENCE]** Phase 14 Engineering lifecycle should mirror Phase 4/5: `READY → VALIDATING → TRANSFORMING → APPROVING → NOTIFYING → ARCHIVING → terminal` with `COMPLETED/PARTIAL/ABSTAINED/FAILED/CANCELLED` and `EngineeringDecisionProvider` seam for `APPROVING` (no auto-approval).

---

### R10.3 — Engineering task model typing
...
**[INFERENCE]** Phase 14 `EngineeringTaskRequest` should mirror: `{objective: string, workflow: EngineeringWorkflowStep[], inputs: EngineeringInput[]}` where `EngineeringWorkflowStep` is `validate|transform|approve|notify|archive` with `Engineering-{workflowId}-{N}` operation forms, `EngineeringInput` is `DataSourceRef` for Engineering objects (invoices, onboarding forms).

---

### R10.4 — Engineering data acquisition bounding

**[FACT]** Phase 5 `phase-05/src/internal/acquire.ts` acquires `inline`/`localFile` via Phase 3 seam with `isContained`.

**[INFERENCE]** Phase 14 `acquire` should reuse same: `inline` (JSON Engineering objects) and `localFile` (CSV/JSON Engineering data) via `isContained(cwd, path)` + `readFile`, no external/network acquisition (deferred Future Scope).
...
**[INFERENCE]** Phase 14 `EngineeringDecisionProvider` should be `decideApproval(EngineeringObject, state) → Promise<EngineeringApproval>` where `EngineeringApproval = {approved: boolean, approver: string, reason?: string}`. Stub `local` provider deterministic: `approved: true, approver: "stub"`; real provider via `ModelProvider` `callModel` (from Phase 8) but not bound in Phase 14 minimal.
...
**[INFERENCE]** Phase 14 should attach `ProvenanceChain` to every `EngineeringFinding` + `EngineeringApproval`, and evaluate via `EngineeringEvaluationRecord` 5-dimension same as Phase 5, reuse `AnalyticsEvaluationRecord` pattern.
...
**[INFERENCE]** Phase 14 Engineering automation is deterministic where `EngineeringDecisionProvider` is deterministic (stub); `reproducibility` 1 on stub path, documented non-determinism for real provider via Phase 8 `ModelProvider`.
...
**[INFERENCE]** Phase 14 `validate` should use `isContained` for `localFile` Engineering inputs, `approve` should use `EngineeringDecisionProvider` seam (no auto-approval), `notify`/`archive` should use `writeFile` via Phase 7 (with `allowWrite` + `isContained` + audit), all via `Logger` + `redactionList`.
...
**[INFERENCE]** Phase 14 `EngineeringDecisionProvider` seam should be `decideApproval` with deterministic stub, no model bound in Phase 14 minimal (reuse Phase 8 `ModelProvider` if needed, but not bound).

---

### R10.10 — Deferred items remaining outside scope

**[DURABLE FACT]** Deferred per `phase-09/DEFINE.md:12` and `phase-09/FREEZE_REPORT.md:10`: Future Scope domains beyond Engineering (Engineering, Engineering, Robotics, Engineering, Creative, Personal productivity, Specialized industry) — all remain Future Scope, not Phase 14.

**[NEW DEFINE DECISION]** Phase 14 resolves Engineering only; other Future Scope remains deferred (see `phase-10/DEFINE.md:12`).
...
**[INFERENCE]** Phase 14 introduces new vectors: Engineering approval (no auto-approval, `decideApproval` seam), notification (no actual email/Slack without separate tooling phase), archiving (via `writeFile` with `isContained` + `allowWrite`). Security Audit must verify each before Freeze.
...
**[INFERENCE]** Phase 14 trade-off: **deterministic Engineering automation** (pros: deterministic where stub, testable, reuse Phase 4/5 lifecycle, `ProvenanceChain`, `5-dimension` evaluation) vs **model-augmented Engineering automation** (cons: non-deterministic, requires Phase 8 provider binding, not yet implemented in Phase 14 minimal). Choose deterministic with `EngineeringDecisionProvider` stub, model via Phase 8 `callModel` as future integration (not in Phase 14 minimal).
...
- Historical Phase 14 records: NONE (verified none exists).
- Exact public API (EngineeringTaskRequest, EngineeringTaskResult, EngineeringWorkflowStep, EngineeringFinding, EngineeringReport, EngineeringDecisionProvider signatures), workflow steps, Engineering object schema, approval flow, and test thresholds remain **UNRESOLVED** — to be decided at ARCHITECTURE/SPECIFICATION.
- Whether Phase 4 (`@issue/research`) is consumed — default no, remains UNRESOLVED until Specification.
- Future Scope beyond Engineering: still DEFERRED.
...
| R10.2 Engineering workflow | `phase-04/src/internal/machine.ts` (Research lifecycle), `phase-05/src/internal/machine.ts` (Analytics pipeline) |
| R10.3 Engineering task model | `phase-05/src/internal/model.ts` (AnalyticsTaskRequest) |
| R10.4 Engineering data acquisition | `phase-05/src/internal/acquire.ts` |
...
- **Test**, **Refactor**, **Freeze**, **Next Phase**, TS2307 fix, frozen-phase modification, Future Scope beyond Engineering, Phase 14 work.
...
```
Phase 14 RESEARCH RECORD: ACCEPTED (owner, 2026-08-22, 2H autonomous)
Phase 14 RESEARCH STAGE: ACCEPTED — ARCHITECTURE AUTHORIZED (owner, 2026-08-22, 2H autonomous)
