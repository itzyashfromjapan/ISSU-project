/**
 * ISSU Phase 11 — Education Automation Agents: deterministic lifecycle machine.
...
import type { EducationDecisionProvider } from "./model.js";
...
export async function runEducationTask(
  request: EducationTaskRequest,
  options?: {
    logger?: Logger;
    provider?: EducationDecisionProvider;
    signal?: AbortSignal;
  },
): Promise<EducationTaskResult> {
  const logger =
  ...
  // APPROVING: via EducationDecisionProvider
  const approvals = await Promise.all(
    transformed.map((inp) =>
      provider.decideApproval(inp, {
        status: "APPROVING" as EducationTaskStatus,
      }),
    ),
  );
  ...
  const findings = transformed.map((inp, idx) => ({
    id: `finding-${idx}`,
    text: `Education finding for ${inp.id}: ${request.objective}`,
    provenance: {
      id: `prov-${idx}`,
      sourceIds: [inp.id],
      steps: [
        {
          kind: "education",
          ref: `Education-${request.workflow[0]?.target ?? "workflow"}-${idx}`,
  ...
  const report = {
    id: "report-0",
    text: `Education report for ${request.objective}: ${findings.length} findings, approved=${allApproved}`,
  ...
  const state: EducationTaskStatus = partial ? "PARTIAL" : "COMPLETED";
  logger.info("Education.audit", {
    objective: request.objective,
    state,
