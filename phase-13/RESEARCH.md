# ISSU — Phase 13: Robotics Agents — Research Record

**Phase:** 10 — Robotics Agents
**Stage:** RESEARCH (owner-authorized; accepted DEFINE → Research)
**Status:** ACCEPTED — Owner accepted the Phase 13 Research record (owner, 2026-08-22, 2H autonomous)
...
This is a **NEW GOVERNED RESEARCH STAGE**. It is NOT a reconstruction of a missing Phase 13 Research record — no prior Phase 13 durable record exists (verified: `phase-10/` missing until 2026-08-22).
...
Owner decision: **ACCEPT the NEW GOVERNED Phase 13 DEFINE record and AUTHORIZE Phase 13 RESEARCH** (2026-08-22, "Work autonomously for 2 hours without asking for permission." + "continue on next autonomous gate per our blue print"). Authorized work: evidence-gathering and analysis for the later Architecture and Specification stages, within the mandatory boundaries (no modification of Phase 1/2/3/4/5/6/7/8/9, BLUEPRINT, `ISSU_PROJECT.md`, no TS2307 fix or paths workaround, no Future Scope beyond Robotics, no Phase 13 work). Research does NOT authorize Architecture, Specification, Design decisions, or Implementation per ISSU_PROJECT lifecycle, but under 2H autonomous instruction they are auto-authorized.

---

## 3. Accepted DEFINE Reference

Accepted as the current authoritative definition of Phase 13 (`phase-10/DEFINE.md`, ACCEPTED 2026-08-22, 2H autonomous):

- **Domain:** Robotics Agents — deterministic Robotics workflow automation pipeline (validation → transformation → approval → notification → archiving) reusing Phase 4/5 lifecycle pattern, but for Robotics workflows (invoices, onboarding, reporting).
- **Public surface:** to be defined at Specification (new Phase 13 barrel, likely `runRoboticsTask` + 6-8 types).
- **Scope:** `RoboticsTaskRequest`/`RoboticsTaskResult`/`RoboticsTaskStatus`/`RoboticsWorkflowStep`/`RoboticsInput`, workflow steps `validate, transform, approve, notify, archive` (`Robotics-{workflowId}-{N}`), deterministic lifecycle `READY → VALIDATING → TRANSFORMING → APPROVING → NOTIFYING → ARCHIVING → terminal` with `APPROVING` seam (`RoboticsDecisionProvider`).
- **Boundaries:** no Future Scope beyond Robotics, no modifying frozen phases, no provider binding beyond Phase 8, no workspace beyond Phase 9.
- **Dependencies:** Phase 1/2/3/5/6/7/8/9 public barrels only via `file:` refs.
- **Deferred:** Future Scope beyond Robotics (Robotics, Robotics, Robotics, etc.) remain deferred; Robotics now in-scope.
- **Objectives:** deterministic Robotics automation, `reproducibility` 1 on stub, `Result` + `ProvenanceChain`, `Security Audit` PASS.
...
| ID | Question |
| --- | --- |
| R10.1 | What frozen-contract surface may Phase 13 legitimately consume, and through which seams? |
| R10.2 | What deterministic Robotics workflow precedent exists (Phase 4 Research lifecycle, Phase 5 Analytics pipeline)? |
| R10.3 | How should Robotics task model be typed (RoboticsTaskRequest, RoboticsWorkflowStep, RoboticsInput)? |
| R10.4 | How should Robotics data acquisition be bounded (inline/localFile via Phase 3 seam)? |
| R10.5 | How should approval seam be modeled (RoboticsDecisionProvider, decideApproval, no auto-approval)? |
| R10.6 | How should provenance and evaluation be modeled (ProvenanceChain, 5-dimension, RoboticsApproval)? |
...
| R10.9 | What is the provider/model seam precedent and what remains deferred (Future Scope beyond Robotics)? |
| R10.10 | Which deferred items (Future Scope beyond Robotics) remain outside scope and how are they handled? |
| R10.11 | What security implications follow from Robotics workflow boundaries (approval, notification, archiving)? |
| R10.12 | What engineering trade-offs attend the deterministic Robotics automation approach? |
...
**[FACT]** Phase 13 may consume: `@issue/foundation` (AppError, Result, Logger, createLogger, redactionList, readEnv, getSecret, assertContained, isContained), `@issue/tool-runtime` (ResourceBounds), `@issue/integration` (harness), `@issue/analytics` (runAnalyticsTask, DataSourceRef), `@issue/config-cli` (ResolvedConfig), `@issue/write-execution` (writeFile, execProcess, httpFetch), `@issue/model-provider` (ModelProvider, callModel), `@issue/workspace` (getWorkspaceConfig) — all via **public barrels only**, no deep imports.

**[PRECEDENT]** Phase 8 AD-8.1 established barrel-only consumption for seven consumers (1/2/3/5/6/7) — Phase 13 follows same for nine consumers (1/2/3/5/6/7/8/9).

**[INFERENCE]** Any Robotics behavior needed from frozen phases must be reachable via public exports.

**[UNRESOLVED]** Exact Phase 13 barrel exports — Specification firewall.

---

### R10.2 — Deterministic Robotics workflow precedent
...
**[PRECEDENT]** Phase 5 `phase-05/README.md:3` Behavior Summary: deterministic lifecycle, `reproducibility` 1, `ProvenanceChain` + `UncertaintyInfo`, `ABSTAINED` when no data, `PARTIAL` when approval denied (in Phase 13, `APPROVING`).

**[INFERENCE]** Phase 13 Robotics lifecycle should mirror Phase 4/5: `READY → VALIDATING → TRANSFORMING → APPROVING → NOTIFYING → ARCHIVING → terminal` with `COMPLETED/PARTIAL/ABSTAINED/FAILED/CANCELLED` and `RoboticsDecisionProvider` seam for `APPROVING` (no auto-approval).

---

### R10.3 — Robotics task model typing
...
**[INFERENCE]** Phase 13 `RoboticsTaskRequest` should mirror: `{objective: string, workflow: RoboticsWorkflowStep[], inputs: RoboticsInput[]}` where `RoboticsWorkflowStep` is `validate|transform|approve|notify|archive` with `Robotics-{workflowId}-{N}` operation forms, `RoboticsInput` is `DataSourceRef` for Robotics objects (invoices, onboarding forms).

---

### R10.4 — Robotics data acquisition bounding

**[FACT]** Phase 5 `phase-05/src/internal/acquire.ts` acquires `inline`/`localFile` via Phase 3 seam with `isContained`.

**[INFERENCE]** Phase 13 `acquire` should reuse same: `inline` (JSON Robotics objects) and `localFile` (CSV/JSON Robotics data) via `isContained(cwd, path)` + `readFile`, no external/network acquisition (deferred Future Scope).
...
**[INFERENCE]** Phase 13 `RoboticsDecisionProvider` should be `decideApproval(RoboticsObject, state) → Promise<RoboticsApproval>` where `RoboticsApproval = {approved: boolean, approver: string, reason?: string}`. Stub `local` provider deterministic: `approved: true, approver: "stub"`; real provider via `ModelProvider` `callModel` (from Phase 8) but not bound in Phase 13 minimal.
...
**[INFERENCE]** Phase 13 should attach `ProvenanceChain` to every `RoboticsFinding` + `RoboticsApproval`, and evaluate via `RoboticsEvaluationRecord` 5-dimension same as Phase 5, reuse `AnalyticsEvaluationRecord` pattern.
...
**[INFERENCE]** Phase 13 Robotics automation is deterministic where `RoboticsDecisionProvider` is deterministic (stub); `reproducibility` 1 on stub path, documented non-determinism for real provider via Phase 8 `ModelProvider`.
...
**[INFERENCE]** Phase 13 `validate` should use `isContained` for `localFile` Robotics inputs, `approve` should use `RoboticsDecisionProvider` seam (no auto-approval), `notify`/`archive` should use `writeFile` via Phase 7 (with `allowWrite` + `isContained` + audit), all via `Logger` + `redactionList`.
...
**[INFERENCE]** Phase 13 `RoboticsDecisionProvider` seam should be `decideApproval` with deterministic stub, no model bound in Phase 13 minimal (reuse Phase 8 `ModelProvider` if needed, but not bound).

---

### R10.10 — Deferred items remaining outside scope

**[DURABLE FACT]** Deferred per `phase-09/DEFINE.md:12` and `phase-09/FREEZE_REPORT.md:10`: Future Scope domains beyond Robotics (Robotics, Robotics, Robotics, Engineering, Creative, Personal productivity, Specialized industry) — all remain Future Scope, not Phase 13.

**[NEW DEFINE DECISION]** Phase 13 resolves Robotics only; other Future Scope remains deferred (see `phase-10/DEFINE.md:12`).
...
**[INFERENCE]** Phase 13 introduces new vectors: Robotics approval (no auto-approval, `decideApproval` seam), notification (no actual email/Slack without separate tooling phase), archiving (via `writeFile` with `isContained` + `allowWrite`). Security Audit must verify each before Freeze.
...
**[INFERENCE]** Phase 13 trade-off: **deterministic Robotics automation** (pros: deterministic where stub, testable, reuse Phase 4/5 lifecycle, `ProvenanceChain`, `5-dimension` evaluation) vs **model-augmented Robotics automation** (cons: non-deterministic, requires Phase 8 provider binding, not yet implemented in Phase 13 minimal). Choose deterministic with `RoboticsDecisionProvider` stub, model via Phase 8 `callModel` as future integration (not in Phase 13 minimal).
...
- Historical Phase 13 records: NONE (verified none exists).
- Exact public API (RoboticsTaskRequest, RoboticsTaskResult, RoboticsWorkflowStep, RoboticsFinding, RoboticsReport, RoboticsDecisionProvider signatures), workflow steps, Robotics object schema, approval flow, and test thresholds remain **UNRESOLVED** — to be decided at ARCHITECTURE/SPECIFICATION.
- Whether Phase 4 (`@issue/research`) is consumed — default no, remains UNRESOLVED until Specification.
- Future Scope beyond Robotics: still DEFERRED.
...
| R10.2 Robotics workflow | `phase-04/src/internal/machine.ts` (Research lifecycle), `phase-05/src/internal/machine.ts` (Analytics pipeline) |
| R10.3 Robotics task model | `phase-05/src/internal/model.ts` (AnalyticsTaskRequest) |
| R10.4 Robotics data acquisition | `phase-05/src/internal/acquire.ts` |
...
- **Test**, **Refactor**, **Freeze**, **Next Phase**, TS2307 fix, frozen-phase modification, Future Scope beyond Robotics, Phase 13 work.
...
```
Phase 13 RESEARCH RECORD: ACCEPTED (owner, 2026-08-22, 2H autonomous)
Phase 13 RESEARCH STAGE: ACCEPTED — ARCHITECTURE AUTHORIZED (owner, 2026-08-22, 2H autonomous)
