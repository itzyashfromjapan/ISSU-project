/**
 * ISSU Phase 13 — Robotics Automation Agents: deterministic lifecycle machine.
...
import type { RoboticsDecisionProvider } from "./model.js";
...
export async function runRoboticsTask(
  request: RoboticsTaskRequest,
  options?: {
    logger?: Logger;
    provider?: RoboticsDecisionProvider;
    signal?: AbortSignal;
  },
): Promise<RoboticsTaskResult> {
  const logger =
  ...
  // APPROVING: via RoboticsDecisionProvider
  const approvals = await Promise.all(
    transformed.map((inp) =>
      provider.decideApproval(inp, {
        status: "APPROVING" as RoboticsTaskStatus,
      }),
    ),
  );
  ...
  const findings = transformed.map((inp, idx) => ({
    id: `finding-${idx}`,
    text: `Robotics finding for ${inp.id}: ${request.objective}`,
    provenance: {
      id: `prov-${idx}`,
      sourceIds: [inp.id],
      steps: [
        {
          kind: "robotics",
          ref: `Robotics-${request.workflow[0]?.target ?? "workflow"}-${idx}`,
  ...
  const report = {
    id: "report-0",
    text: `Robotics report for ${request.objective}: ${findings.length} findings, approved=${allApproved}`,
  ...
  const state: RoboticsTaskStatus = partial ? "PARTIAL" : "COMPLETED";
  logger.info("Robotics.audit", {
    objective: request.objective,
    state,
